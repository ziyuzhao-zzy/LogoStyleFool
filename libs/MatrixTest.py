...
class CNN(nn.Module):
    ...
    def forward(self, x, masks, style=False):
        ...
        for i in range(color_code_number):
            ...
            mask = cv2.resize(mask.numpy(), (xw, xh),
        ...
        for i in range(color_code_number):
            mask = masks[i].clone().squeeze(0)
            mask = cv2.resize(
                mask.numpy(), (w, h), interpolation=cv2.INTER_NEAREST)
            mask = torch.FloatTensor(mask)
            mask = mask.long()
            if (torch.sum(mask) >= 10):
                mask = mask.view(-1)
                ...
                covMatrix = torch.mm(
                    selectFeature, selectFeature.transpose(
                        0, 1)).div(tN)
...
class MulLayer(nn.Module):
    ...
    def forward(self, cF, sF, cmasks, smasks):
        ...
        for i in range(color_code_number):
            ...
            cmask = cv2.resize(cmask.numpy(), (cw, ch),
                               interpolation=cv2.INTER_NEAREST)
            cmask = torch.FloatTensor(cmask)
            cmask = cmask.long()
            smask = cv2.resize(smask.numpy(), (sw, sh),
                               interpolation=cv2.INTER_NEAREST)
            ...
            if (torch.sum(cmask) >= 10 and torch.sum(smask) >= 10
               and (i in sMatrices) and (i in cMatrices)):
                ...
                compress_content_select = torch.index_select(
                    compress_content, 1, fgcmask)
                ...
                sMean_select = torch.index_select(
                    sMean.view(sc, -1), 1, fgcmask)
